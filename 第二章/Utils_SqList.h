#pragma once
//包含了原C2-1.h,bo2-1.cpp,C1.h
#include "Basic.h"
//typedef int ElemType;//！！！如果注释掉这行，则需要在include此文件之前typedef，否则不能注释掉这行！！！
//ElemType 在调用时定义
//From old C2-1.h,定义了动态线性表，可变长度
#define LIST_INIT_SIZE 10 // 线性表存储空间的初始分配量
#define LIST_INCREMENT 2 // 线性表存储空间的分配增量
struct SqList
{
	ElemType* elem; // 存储空间基址,首地址
	int length; // 当前顺序表的总长度
	int listsize; // 当前分配的存储容量(以sizeof(ElemType)为单位)，允许的最大存储空间
};
// From old bo2-1.cpp 顺序存储的线性表(存储结构由c2-1.h定义)的基本操作(12个)，包括算法2.3～2.6
...
Status ListEmpty(SqList L)//isListEmpt would be better
{ // 初始条件：顺序线性表L已存在。
...
{ // 初始条件：顺序线性表L已存在，1≤i≤ListLength(L)
  // 操作结果：删除L的第i个数据元素，并用e返回其值，L的长度减1
	...
	for (++p; p <= q; ++p) // 被删除元素之后的元素右移(由被删除元素的后继元素开始移)，第一个++p就执行一次
		* (p - 1) = *p;
	L.length--; // 表长减1
	return OK;
}
